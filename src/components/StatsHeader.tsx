...
interface StatsProps {
  ...
  activeStatusFilter?: 'total' | 'completed' | 'pending';
  activeDetailFilter?: 'priority-1' | 'priority-2' | 'priority-3' | 'institutions' | 'people' | null;
  onFilterTotal?: () => void;
...
export function StatsHeader({
  ...
  type StatusCardId = 'total' | 'completed' | 'pending';
  ...
  type DetailCardId = 'priority-1' | 'priority-2' | 'priority-3' | 'institutions' | 'people';
  ...
  const renderDetailCard = (card: DetailCard) => {
    const isActive = activeDetailFilter === card.id;
    return (
      <button
        key={card.id}
        type="button"
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3">
      <div className="grid grid-cols-3 gap-3 flex-1">{statusCards.map(renderStatusCard)}</div>
      <div className="hidden sm:block w-px bg-slate-300 mx-1" />
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 flex-[2]">{detailCards.map(renderDetailCard)}</div>
    </div>
  );
}
